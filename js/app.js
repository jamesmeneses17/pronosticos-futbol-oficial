...
(function () {
  ...
  function tarjetaPronostico(titulo, p) {
    ...
    const btnGroup = crear('div', 'acciones-asignar');
    btnGroup.style.marginTop = '1rem';
    btnGroup.style.display = 'flex';
    btnGroup.style.gap = '0.5rem';
    
    const labelAsignar = crear('span', null, 'Llevar a registro: ');
    labelAsignar.style.fontSize = '0.85rem';
    labelAsignar.style.color = 'var(--c-text-muted)';
    labelAsignar.style.alignSelf = 'center';
    btnGroup.appendChild(labelAsignar);
    
    ['Gemini', 'ChatGPT', 'Claude'].forEach(ia => {
      const btn = crear('button', 'btn', ia);
      btn.style.padding = '0.2rem 0.5rem';
      btn.style.fontSize = '0.8rem';
      btn.addEventListener('click', async () => {
        const idTextarea = 'reg-' + ia.toLowerCase();
        const textarea = $(idTextarea);
        if (textarea) {
          // Obtener la probabilidad para añadirla al texto (ej: "Over 0.5 goles (92%)")
          const probTexto = p.probability ? ' (' + Math.round(p.probability * 100) + '%)' : '';
          textarea.value = R.nombreDe(p.market) + probTexto;
          
          if (ultimoAnalisis && ultimoAnalisis.json.match) {
             const m = ultimoAnalisis.json.match;
             if ($('reg-local') && !$('reg-local').value) $('reg-local').value = m.homeTeam;
             if ($('reg-visitante') && !$('reg-visitante').value) $('reg-visitante').value = m.awayTeam;
             if ($('reg-competicion') && !$('reg-competicion').value) $('reg-competicion').value = m.competition;
          }
          
          if (window.guardarPronostico) {
            await window.guardarPronostico(true);
  ...
  function cargarEjemplo() {
    const valores = {
      'in-local': 'Equipo Local', 'in-visitante': 'Equipo Visitante',
      'in-competicion': 'Liga de ejemplo', 'in-media-liga': '2.55',
      'l-pj': '10', 'l-gf': '1.80', 'l-gc': '0.90', 'l-forma': '11', 'l-bajas': '1', 'l-descanso': '6',
      'v-pj': '10', 'v-gf': '1.10', 'v-gc': '1.50', 'v-forma': '6', 'v-bajas': '2', 'v-descanso': '4',
      'cuota-LOCAL': '1.75', 'cuota-EMPATE': '3.60', 'cuota-VISITANTE': '4.60',
      'cuota-DOBLE_1X': '1.20', 'cuota-OVER_2_5': '1.95', 'cuota-UNDER_2_5': '1.85',
      'cuota-BTTS_SI': '2.05', 'cuota-BTTS_NO': '1.72'
    };
  ...
  function cargarDesdeJsonCuotas() {
    const msg = $('json-cuotas-msg');
    if (!msg) return;
    const input = $('in-json-cuotas').value.trim();
    if (!input) {
      msg.className = 'form-msg warn';
      msg.textContent = 'Pega el JSON primero.';
      return;
    ...
    try {
      if (data.cuotas) {
        const c = data.cuotas;
        if (c.resultado) {
          if (esNum(c.resultado.gana_local)) $('cuota-LOCAL').value = c.resultado.gana_local;
          if (esNum(c.resultado.empate)) $('cuota-EMPATE').value = c.resultado.empate;
          if (esNum(c.resultado.gana_visitante)) $('cuota-VISITANTE').value = c.resultado.gana_visitante;
        }
        if (c.doble_oportunidad) {
          if (esNum(c.doble_oportunidad['1x'])) $('cuota-DOBLE_1X').value = c.doble_oportunidad['1x'];
          if (esNum(c.doble_oportunidad.x2)) $('cuota-DOBLE_X2').value = c.doble_oportunidad.x2;
          if (esNum(c.doble_oportunidad['12'])) $('cuota-DOBLE_12').value = c.doble_oportunidad['12'];
        }
        if (c.goles) {
          if (esNum(c.goles.over_0_5)) $('cuota-OVER_0_5').value = c.goles.over_0_5;
          if (esNum(c.goles.over_1_5)) $('cuota-OVER_1_5').value = c.goles.over_1_5;
          if (esNum(c.goles.over_2_5)) $('cuota-OVER_2_5').value = c.goles.over_2_5;
          if (esNum(c.goles.over_3_5)) $('cuota-OVER_3_5').value = c.goles.over_3_5;
          if (esNum(c.goles.under_1_5)) $('cuota-UNDER_1_5').value = c.goles.under_1_5;
          if (esNum(c.goles.under_2_5)) $('cuota-UNDER_2_5').value = c.goles.under_2_5;
          if (esNum(c.goles.under_3_5)) $('cuota-UNDER_3_5').value = c.goles.under_3_5;
        }
        if (c.ambos_marcan) {
          if (esNum(c.ambos_marcan.si)) $('cuota-BTTS_SI').value = c.ambos_marcan.si;
          if (esNum(c.ambos_marcan.no)) $('cuota-BTTS_NO').value = c.ambos_marcan.no;
        }
        if (c.goles_del_local) {
          if (esNum(c.goles_del_local.mas_0_5)) $('cuota-LOCAL_OVER_0_5').value = c.goles_del_local.mas_0_5;
          if (esNum(c.goles_del_local.mas_1_5)) $('cuota-LOCAL_OVER_1_5').value = c.goles_del_local.mas_1_5;
        }
        if (c.goles_del_visitante) {
          if (esNum(c.goles_del_visitante.mas_0_5)) $('cuota-VISITANTE_OVER_0_5').value = c.goles_del_visitante.mas_0_5;
          if (esNum(c.goles_del_visitante.mas_1_5)) $('cuota-VISITANTE_OVER_1_5').value = c.goles_del_visitante.mas_1_5;
        }
      }
      
      msg.className = 'form-msg ok';
      msg.textContent = 'Cuotas cargadas con éxito.';
    } catch (e) {
      msg.className = 'form-msg warn';
      msg.textContent = 'Error al procesar el JSON: ' + e.message;
    }
  ...
  function init() {
    ...
    if ($('btn-cargar-json-cuotas')) {
      $('btn-cargar-json-cuotas').addEventListener('click', cargarDesdeJsonCuotas);
    }
    $('btn-limpiar').addEventListener('click', limpiar);
    $('in-globales').addEventListener('change', sincronizarLocalia);

    sincronizarLocalia();
    // cargarPrompt(); se quitó la pestaña Prompt
  }
