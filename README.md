...
2. **La aplicación** — un motor que produce ese mismo JSON por cálculo, a partir de los datos que
   tú introduces, más un validador que comprueba cualquier JSON contra las reglas de §23.
...
Sirve la carpeta por HTTP (el panel del prompt lee `PROMPT_MAESTRO.md` y el protocolo `file://`
lo bloquea):

```bash
python -m http.server 8090 --directory "C:/Users/Jamex/Downloads/sistema-futbol"
```

Y abre <http://localhost:8090>. Desde Claude Code, la configuración `sistema-futbol` de
`.claude/launch.json` levanta ese mismo servidor.
...
| **JSON** | La salida de §25 tal cual, para copiar o descargar. |
| **Validador** | Pega cualquier JSON —por ejemplo el que devuelva un modelo con el prompt maestro— y comprueba §23. |
| **Prompt** | El prompt maestro, para copiarlo de una vez. |
...
```
sistema-futbol/
├── PROMPT_MAESTRO.md   Prompt completo (§1–§32)
├── index.html          Interfaz
├── css/styles.css      Sistema de diseño
└── js/
    ├── modelo.js       Poisson bivariado + Dixon-Coles, derivación de mercados
    ├── reglas.js       EV, riesgo, confianza, selección y ensamblado del JSON
    ├── validador.js    Validación §23 sobre cualquier objeto
    └── app.js          Interfaz, render y portapapeles
```
